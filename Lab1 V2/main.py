...
class UI:
    ...
    def in_out_degree(self):
        ...
        print("Out degree of vertex", vertex,
              "is :", len(self.graph.out_degree(vertex)))
        print("In degree of vertex", vertex,
              "is :", len(self.graph.in_degree(vertex)))
    ...
    def parse_out_vertices(self):
        vertex = int(input("Vertex > "))
        print(self.graph.out_verteces(vertex))
...
def generate_graph():
    num_verteces = int(input("Number of vertices > "))
    num_edges = int(input("Number of edges > "))

    graph = [num_verteces, num_edges]
    edges = []
    while len(edges) < num_edges:
        v1 = random.choice(range(num_verteces))
        v2 = random.choice(range(num_verteces))
